        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                if (Usernames != null)
                {
                    ...
                    UserList = Usernames.Split(',').ToList();

                    if (UserList != null)
                    {
                        int UserCount = UserList.Count;
                        Console.WriteLine("[+] Successfully collected " + UserCount + " usernames from User input.");

                    }
                    else
                    {
                        Console.WriteLine("[-] Failed to create a list the usernames from user input; the script will exit.");
                        System.Environment.Exit(1);
                    }
                } 
                else
